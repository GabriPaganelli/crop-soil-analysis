...
logN   & $-0.051$ & $0.044$ & $[-0.124,\; +0.021]$ & trascurabile (CI include 0) \\
...
\medskip
Il coefficiente $b_{\SOC} = \eta_{\SOC}/\psi_{\SOC} = 0.645$ (CI\,90\%: $[0.388,\; 0.990]$):
per ogni unità di deviazione standard nel livello del campo,
la slope di decadimento verticale cambia di circa $0.65\,\psi_{\SOC}$.
...
\texttt{scripts/01b\_eda\_tessitura.R} & EDA composizionale tessitura \\
\texttt{scripts/02\_eda\_forma\_icc.R} & forma funzionale, ICC, motivazione M-SP \\
\texttt{stan/m4rr\_v2\_ri\_slope\_mu.stan} & codice Stan M-SP \\
\texttt{stan/fit\_msp.rds} & fit MCMC salvato \\
\texttt{scripts/06\_model\_msp.R} & fit M-SP, diagnostica, LOO \\
\texttt{scripts/06b\_model\_msp\_mv.R} & robustezza: residui multivariati (M-SP-MV) \\
\texttt{scripts/08\_selezione\_variabili.R} & selezione variabili (projpred) \\
