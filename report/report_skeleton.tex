...
\begin{figure}[htbp]
  ...
  \caption{Scatter intercetta vs.\ slope OLS per i 40 campi (SOC, N, P).
    La figura mostra $r_\mathrm{OLS}\approx 0.39$ per SOC (stima per campo
    con $n=4$--6, rumore di misura attenua la correlazione).
    \todo{Nel testo: la stima BLUP da lmer d\`a $r\approx 0.93$ --- valore
    pi\`u robusto all'attenuazione. Valutare se rigenarare la figura con i
    BLUP (script 02, sezione 0c).}}
  \label{fig:scatter}
\end{figure}
