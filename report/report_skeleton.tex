...
\title{%
  ...
  {\LARGE\bfseries Profili verticali di nutrienti del suolo}\\[4pt]
...
\begin{figure}[htbp]
  \centering
  \noindent\todo{CANDIDATO RIMOZIONE --- lasciare per ora, segnalare nel testo}\
...
\begin{figure}[htbp]
  ...
  \caption{Relazione intercetta--slope per i 40 campi (BLUP lmer,
    \texttt{logBottom} centrato).
    SOC: $r=+0.62$ ($p<0.001$), correlazione moderata--forte che motiva
    il modello M-SP.
    N: $r\approx +0.11$, nessun pattern sistematico.
    P: fit singolare nel modello lmer
    ($\widehat{\mathrm{Var}}(\text{slope})\approx 0$), coerente con $\eta_P$
    borderline in M-SP.}
  \label{fig:scatter}
\end{figure}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lccc@{}}
    ...
    $\alpha_r$ &
      ...
      $\mathbf{-2.594}\ \CI{-2.800}{-2.381}$ &
      $\mathbf{-2.611}\ \CI{-2.959}{-2.257}$ \\[2pt]
    $\psi_r$ &
      $\mathbf{0.323}\ \CI{0.231}{0.436}$ &
      $\mathbf{0.197}\ \CI{0.131}{0.274}$ &
      $\mathbf{0.392}\ \CI{0.293}{0.515}$ \\[2pt]
    $\boldsymbol{\eta_r}$ &
      $\mathbf{0.209}\ \CI{0.125}{0.302}$ &
      $-0.051\ \CI{-0.124}{0.021}$ &
      $\mathbf{0.096}\ \CI{0.019}{0.179}$ \\[2pt]
    $b_r = \eta_r/\psi_r$ &
      $\mathbf{0.645}\ \CI{0.388}{0.990}$ &
      $-0.252\ \CI{-0.748}{0.103}$ &
      $\mathbf{0.244}\ \CI{0.048}{0.461}$ \\[2pt]
    $\sigma_r$ &
      $\mathbf{0.531}\ \CI{0.486}{0.582}$ &
      $\mathbf{0.345}\ \CI{0.316}{0.378}$ &
      $\mathbf{0.548}\ \CI{0.502}{0.599}$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[htbp]
  \centering
  \noindent\todo{VALUTARE rimozione (incerto)}\
...
\subsection{Indipendenza condizionale dei residui (M-SP-MV)}

Per verificare se i residui dei tre nutrienti alla stessa osservazione
covariassero, M-SP \`e stato esteso a un modello con residui multivariati
(M-SP-MV): $\vec{\varepsilon}_{ij} \sim \mathcal{N}_3(\vec{0},\,\Sigma)$
con $\Sigma = \mathrm{diag}(\vec\sigma)\,\Omega\,\mathrm{diag}(\vec\sigma)$
e prior $\Omega \sim \mathrm{LKJ}(2)$.

Le tre correlazioni residue a posteriori sono tutte vicine a zero:
$\hat\rho_{\SOC\text{-}N} = 0.067\ [-0.063, 0.197]$,
$\hat\rho_{\SOC\text{-}P} = 0.001\ [-0.122, 0.126]$,
$\hat\rho_{N\text{-}P}   = 0.021\ [-0.105, 0.148]$.
I parametri $\eta_r$ sono invariati ($\leq 0.005$ di variazione in mediana).
Il confronto LOO indica che M-SP \`e migliore di M-SP-MV
($\Delta\text{ELPD} = +4.7$, SE~$=1.4$, $|z|=3.4$):
aggiungere le correlazioni residue peggiora la predizione perch\'e
introduce parametri con segnale nullo.

\textbf{Conclusione}: i tre nutrienti sono condizionalmente indipendenti
dato il modello M-SP. La co-variazione osservata tra SOC, N e P \`e
interamente catturata dai predittori comuni (profondit\`a, tessitura, campo).
M-SP rimane il modello finale.
