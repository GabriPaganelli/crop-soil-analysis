...
Per ogni risposta $r \in \{1, 2, 3\}$ (corrispondente a SOC, N, P)
e osservazione $i$ nel campo $j$:
%
\[
...
\[
  \mu_{r,ij} =
    ...
    +\;
    \underbrace{u_{rj}}_{\text{intercetta casuale}}
    +\;
    \underbrace{v_{rj} \cdot \log B_i}_{\text{slope casuale}}
    +\;
    \underbrace{\boldsymbol{\gamma}_r^\top \mathbf{x}_{W,i}}_{\text{effetti fissi within}}
    +\;
    \underbrace{\boldsymbol{\beta}_r^\top \mathbf{x}_{B,j}}_{\text{effetti fissi between}}
\]
%
dove il vettore degli effetti casuali di campo è modellato congiuntamente:
%
\[
  \mathbf{b}_j
  = \bigl(u_{1j},\, v_{1j},\, u_{2j},\, v_{2j},\, u_{3j},\, v_{3j}\bigr)^\top
  \sim \mathcal{N}_6\!\bigl(\mathbf{0},\; \Sigma\bigr),
\]
%
\[
  \Sigma
  = \operatorname{diag}(\boldsymbol{\tau})\;\Omega\;\operatorname{diag}(\boldsymbol{\tau}),
  \qquad
  \Omega \sim \mathrm{LKJ}(2).
\]
%
Le componenti di $\mathbf{b}_j$ sono ordinate per risposta:
$(u_{1j}, v_{1j})$ per SOC, $(u_{2j}, v_{2j})$ per N,
$(u_{3j}, v_{3j})$ per P.
