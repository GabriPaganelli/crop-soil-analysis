// =============================================================================
// m_sp_rirs_A.stan  —  Modello A (ridotto): M-SP-RIRS senza beta e senza Texture1
//
// Confronto parsimonia con M-SP-RIRS (script 13_confronto_modelli.R):
//   - Nessun effetto between-field (management rimosso)
//   - X_W senza Texture1 (ILR1): K_W = 4 {logBottom, Texture2, BulkDensity, PH}
//
// La struttura bivariata RI+RS rimane identica a M-SP-RIRS.
// Domanda: management e ILR1 contribuiscono predittivamente?
//
// X_W (K_W=4): logBottom[1], Texture2[2], BulkDensity[3], PH[4]
// TOTALE PARAMETRI: 6J + 30 scalari  (J=40: 270)
// =============================================================================
...
model {
...

  {
    ...
    for (n in 1:N) {
      ...
      real mu_SOC = alpha_SOC + u_SOC[1,j] + u_SOC[2,j]*logB_n + gSOC[n];
      real mu_N   = alpha_N   + u_N[1,j]   + u_N[2,j]  *logB_n + gN[n];
      real mu_P   = alpha_P   + u_P[1,j]   + u_P[2,j]  *logB_n + gP[n];
